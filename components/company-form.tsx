...
const EXCLUSION_FIELDS: Field[] = [
  {
    key: "excludeIndustries",
    label: "Отрасли не берём",
    placeholder: "напр. гемблинг, оружие, крипто-скам",
  },
  {
    key: "excludeRegions",
    label: "Регионы не берём",
    placeholder: "напр. РФ, СНГ, страны с санкциями",
  },
  {
    key: "excludeSizes",
    label: "Размеры не берём",
    placeholder: "напр. <10 сотрудников, enterprise 5000+",
  },
  {
    key: "stopFactors",
    label: "Стоп-факторы",
    ...
    placeholder:
      "напр. нет бюджета, уже наш клиент, банкротство, судебные иски…",
    type: "textarea",
    full: true,
  },
];
...
const PRIORITY_FIELDS: Field[] = [
  ...
  },
  {
    ...
    placeholder:
      "напр. совпадение с ICP, боль в открытых источниках, кейс из профиля…",
    type: "textarea",
    full: true,
  },
  {
...
function Cell({
  ...
  return (
    <div
      className={`flex flex-col gap-1.5 rounded-lg border border-border bg-card p-3.5 ${field.full ? "sm:col-span-2" : ""}`}
    >
      <label
        htmlFor={field.key}
          ...
          <div className="rounded-lg border-l-2 border-accent bg-accent/10 p-3.5">
            <div className="mb-3 flex items-start gap-2">
              ...
              <p className="text-xs leading-snug text-accent-foreground">
                <span className="font-semibold">Фильтры исключения.</span> Самый
                важный блок: то, что сюда попало, режется до расчёта приоритета
                — агент не тратит на это разбор.
              </p>
            </div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
